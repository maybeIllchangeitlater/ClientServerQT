...
void StringRepository::insertString(const std::string &stringToInsert) {
  ...
  std::string sql = std::string("INSERT INTO ") + database::STRING_TABLE +
                    " (" + database::STRING_TABLE_TEXT + ") VALUES (" +
                    task.quote(stringToInsert) + ")";
  try {
    task.exec(sql);
    task.commit();
...
pqxx::result StringRepository::getStringCount() {
  ...
  std::string sql = std::string("SELECT ") + database::ID + " FROM " +
                    database::STRING_TABLE + " ORDER BY id DESC LIMIT 1";
  try {
