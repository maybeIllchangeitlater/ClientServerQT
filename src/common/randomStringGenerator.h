...
class RandomStringGenerator {
  constexpr static const char CHARNUMSET[] =
 ...
 public:
  /**
   * @brief RandomStringGenerator создает объект класса предназначенного для
   * генерации случайных строк
   */
  RandomStringGenerator()
      : _gen(std::random_device()()),
        _lengthDistribution(MIN_STRING_LENGTH, MAX_STRING_LENGTH),
        _charNumDistribution(0, sizeof(CHARNUMSET) - 2),
        _numOnlyDistribution(0, NUMBERS_END_AFTER) {}

  /**
   * @brief generateNumCharString генерирует строку из случайных латинских букв
   * и цифр длинной 5-50
  ...
  /**
   * @brief generateCharOnlyString генерирует строку из случайных цифр длинной
   * 5-50
   * @return сгенерированная строку
   */
 ...
 private:
  /**
   * @brief generateString генерирует строку основываясь на массиве латинский
   * цифр и букв(CHARNUMSET) и пришедшую дистрибуцию
   * @param dist - дистрибуция(от минимума до максимума), _charNumDistribution
   * для цифр и букв, _charOnlyDistribution для цифр
   * @return сгенерированная строку
   */
